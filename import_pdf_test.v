...
fn test_imports_pdf_stream_containing_endobj_literal() {
	source := '%PDF-1.4\n1 0 obj\n<< /Type /Catalog /Pages 2 0 R >>\nendobj\n2 0 obj\n<< /Type /Pages /Kids [3 0 R] /Count 1 >>\nendobj\n3 0 obj\n<< /Type /Page /Parent 2 0 R /MediaBox [0 0 200 200] /Resources << >> /Contents 5 0 R >>\nendobj\n5 0 obj\n<< /Length 34 >>\nstream\nBT (literal endobj sentinel) Tj ET\nendstream\nendobj\ntrailer\n<< /Root 1 0 R /Size 6 >>\nstartxref\n0\n%%EOF\n'
	mut doc := new_document()
	imported := doc.add_pdf_pages_from_bytes(source.bytes())!
	body := doc.render()
	assert imported == 1
	assert body.contains('literal endobj sentinel')
	assert body.contains('/Count 1')
}

fn test_imports_pdf_stream_using_length_when_delimiter_words_are_literal() {
	data := 'BT (literal endstream endobj marker) Tj ET'
	source := '%PDF-1.4\n1 0 obj\n<< /Type /Catalog /Pages 2 0 R >>\nendobj\n2 0 obj\n<< /Type /Pages /Kids [3 0 R] /Count 1 >>\nendobj\n3 0 obj\n<< /Type /Page /Parent 2 0 R /MediaBox [0 0 200 200] /Resources << >> /Contents 5 0 R >>\nendobj\n5 0 obj\n<< /Length ${data.len} >>\nstream\n${data}\nendstream\nendobj\ntrailer\n<< /Root 1 0 R /Size 6 >>\nstartxref\n0\n%%EOF\n'
	mut doc := new_document()
	imported := doc.add_pdf_pages_from_bytes(source.bytes())!
	body := doc.render()
...
fn test_import_removes_page_parent_ref_split_by_newline() {
	...
	body := doc.render()
	assert imported == 1
	assert body.contains('newline parent export page')
...
fn test_imports_page_object_from_flate_object_stream() {
	source := object_stream_pdf_for_import_test('\nstream\n', '\nendstream\n')!
	mut doc := new_document()
...
fn test_imports_page_object_from_cr_only_flate_object_stream() {
	source := object_stream_pdf_for_import_test('\rstream\r', '\rendstream\r')!
	mut doc := new_document()
	imported := doc.add_pdf_pages_from_bytes(source)!
	body := doc.render()
	assert imported == 1
...
fn object_stream_pdf_for_import_test(stream_marker string, endstream_marker string) ![]u8 {
